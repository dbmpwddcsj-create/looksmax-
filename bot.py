    ...
    if repeated:
        ...
        rows.append([
            ...
            InlineKeyboardButton(
                ...
                callback_data=f"repeat_rate:{user_id}"
            )
...
async def show_rating_profile(
    ...
    if repeated:
        ...
        previous_rating = await db.get_rating(
...
async def rate_people(
    ...
    # Сначала ВСЕГДА ищем анкету,
    # которую пользователь ещё не оценивал.

    user = await db.random_unrated_profile(
    ...
    # Теперь можно показывать старые.
...
async def rate_specific(
    ...
    # конкретную анкету — показываем предупреждение.
