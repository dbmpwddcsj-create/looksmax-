...
async def next_profile(
    ...
    skipped_ids = list(
        data.get("skipped_profile_ids", [])
    )

    if current_id is not None:
        current_id = int(current_id)

        if current_id not in skipped_ids:
            skipped_ids.append(current_id)

    await state.update_data(
        skipped_profile_ids=skipped_ids
    )
...
async def show_next_profile(
    ...
    # ---------------------------------------------------------
    # СНАЧАЛА ИЩЕМ НОВЫЕ НЕОЦЕНЁННЫЕ АНКЕТЫ
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # НОВЫХ НЕТ — ПЕРЕХОДИМ К УЖЕ ОЦЕНЁННЫМ
    # ---------------------------------------------------------

    rated_excluded = list(excluded_ids)

    profile = await db.get_random_rated_profile(
        user_id,
        exclude_user_ids=rated_excluded,
    )
    ...
    # ---------------------------------------------------------
    # ВООБЩЕ ДРУГИХ АНКЕТ НЕТ
    # ---------------------------------------------------------

    await state.update_data(
        rating_profile_user_id=None
    )

    await message.answer(
        "😔 Других доступных анкет нет.",
        reply_markup=main_menu(),
    )
