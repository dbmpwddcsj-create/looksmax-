...
def look_type_keyboard(
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=f"👤 Главный образ ({label})",
                    callback_data="look:main",
                )
            ],
            [
                InlineKeyboardButton(
                    text="✨ Внешность",
                    callback_data="look:appearance",
                )
            ],
            [
                InlineKeyboardButton(
                    text="👗 Стиль",
                    callback_data="look:style",
                )
            ],
            [
                InlineKeyboardButton(
...
async def menu_rate(
    ...
    # но пользователь всё равно может выбрать новый.
    await state.update_data(
...
async def ask_look_type(
    ...
    await callback.message.edit_text(
        f"Оценка: <b>{float(score):g}/10</b>\n\n"
        "Что именно оцениваем?",
        reply_markup=look_type_keyboard(
            gender
        ),
        parse_mode="HTML",
    )
