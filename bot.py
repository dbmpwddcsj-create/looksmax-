...
async def rate_back(
    ...
    # Проверяем, что пользователь зарегистрирован и у него есть анкета
    ...
    if not profile or profile.get("status") != "active":
        await callback.answer(
            "Сначала создай активную анкету.",
            show_alert=True,
        )
        return

    # Получаем анкету того, кто оценил
    ...
    # Получаем режим оценивания текущего пользователя
    ...
    # Сохраняем данные для оценивания
    await state.update_data(
        rating_mode=mode,
        rating_profile_user_id=rater_id,
        seen_profile_ids=[rater_id],  # чтобы не показывать снова
    )

    # Отправляем профиль и клавиатуру для оценки
...
async def view_rating_table(
    ...
    image = get_table_image_for_gender(
        target_user.get("gender")
    )
    ...
    msg = await callback.message.answer_photo(
        ...
        caption=(
            "📊 <b>Таблица оценки</b>\n\n"
            "Выбери категорию, которая лучше "
            "всего соответствует внешности анкеты."
        ),
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⬅️ Закрыть таблицу",
                        callback_data="close_rating_table",
                    )
                ]
            ]
        ),
    ...
    await state.update_data(
        rating_table_message_id=msg.message_id
    )

    await callback.answer()

@dp.callback_query(
    F.data == "close_rating_table"
)
async def close_rating_table(
    callback: CallbackQuery,
    state: FSMContext,
):
    try:
...
async def send_mutual_match(
    ...
    user_a_data = await db.get_user(
        user_a
    )

    user_b_data = await db.get_user(
        user_b
    )

    username_a = (
        f"@{user_a_data.get('username')}"
        if user_a_data
        and user_a_data.get("username")
        else "пользователь"
    )

    username_b = (
        f"@{user_b_data.get('username')}"
        if user_b_data
        and user_b_data.get("username")
        else "пользователь"
    )
    ...
    try:
        await bot.send_message(
            ...
            (
                ...
                f"Твоя симпатия: {html.escape(username_b)}"
            ),
            parse_mode="HTML",
        )
    except Exception:
    ...
    try:
        await bot.send_message(
            ...
            (
                "❤️ <b>Взаимная симпатия!</b>\n\n"
                f"Твоя симпатия: {html.escape(username_a)}"
            ),
            parse_mode="HTML",
        )
    except Exception:
...
async def add_advice(
    ...
    await callback.message.edit_text(
        (
            "💬 <b>Напиши совет по улучшению внешности.</b>\n\n"
...
@dp.callback_query(
    F.data == "advice_view_profile",
    Advice.text,
)
async def advice_view_profile(
    callback: CallbackQuery,
    state: FSMContext,
):
    data = await state.get_data()
    ...
    # send_profile записал свои сообщения в bot_message_ids.
    # Переносим их в отдельный список, чтобы они удалялись
    # при возврате к написанию совета.
    ...
    await state.update_data(
        advice_profile_message_ids=profile_ids,
        bot_message_ids=[],
    )
...
async def advice_text(
    ...
    # Предполагаем, что create_advice возвращает id созданного совета
    advice_id = await db.create_advice(
    ...
    try:
        await bot.send_message(
            target_id,
            "💬 Тебе оставили новый совет по твоей анкете!",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="👁 Посмотреть совет",
                            callback_data=f"view_advice:{advice_id}",
                        )
                    ]
                ]
            ),
...
# =========================================================
# NEW: VIEW ADVICE HANDLER
# =========================================================

@dp.callback_query(
    F.data.startswith("view_advice:")
)
async def view_advice(
    callback: CallbackQuery,
):
    try:
        advice_id = int(
    ...
    advice = await db.get_advice_by_id(advice_id)
    ...
    message_text = "💬 <b>Совет по твоей анкете:</b>\n\n"
    if score is not None:
        message_text += f"Оценка: <b>{score:g}/10</b>\n\n"
    message_text += html.escape(str(text))
