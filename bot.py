...
def get_profile_photos(profile: dict):
    return db.parse_photo_ids(
...
async def send_photos(
    ...
    photo_ids = get_profile_photos(profile)
...
async def create_profile_start(
    ...
    await callback.message.answer(
        "📸 Отправь фотографии своего лица.\n\n"
        "Можно отправить несколько фотографий.\n"
        "После того как закончишь, нажми «✅ Готово».\n\n"
        "На фотографиях должно быть настоящее лицо.",
...
async def photos_done(
    ...
    if not photo_ids:
        await callback.answer(
            "Сначала отправь хотя бы одну фотографию.",
            show_alert=True,
        )
        return
...
async def profile_weight(
    ...
    if len(photo_ids) == 1:
        photo_value = photo_ids[0]
    else:
        photo_value = json.dumps(
            photo_ids,
            ensure_ascii=False,
        )
...
async def receive_new_gender(
    callback: CallbackQuery,
    state: FSMContext,
):
    gender = (
        "Мужской"
        if callback.data == "change_gender_male"
        else "Женский"
    )
