...
# Telegram file_id фотографии для главного меню.
# Фото не будет использоваться в приветственном сообщении.
MAIN_MENU_PHOTO_ID = os.environ.get("MAIN_MENU_PHOTO_ID")
...
def rating_keyboard(profile_user_id: int):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    text="❤️ Нравится",
                    callback_data=f"like_user:{profile_user_id}",
                )
            ],
            [
                InlineKeyboardButton(
                    text="⏭ Пропустить анкету",
...
def like_notification_keyboard(user_id: int):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👤 Посмотреть анкету",
                    callback_data=f"view_rater:{user_id}",
                )
            ]
        ]
    )
...
def format_username(user: dict | None):
    if not user:
...
async def send_main_menu(
    ...
    """
    Главное меню.
    Фото показывается только здесь.
    В обычных сообщениях фото не используется.
    """
...
@dp.callback_query(F.data == "skip_profile")
...
async def send_rating_profile(
    ...
    # Username здесь специально НЕ показываем.
    # Он появляется только после взаимной симпатии.
    text = (
        "👤 Анкета пользователя\n"
        f"🎂 Возраст: {user.get('age', '—')}\n"
        f"⚧ Пол: {user.get('gender', '—')}\n"
...
async def like_user(
    ...
    if existing:
        await callback.answer(
            "❤️ Ты уже ставил этому пользователю лайк.",
            show_alert=True,
        )
        return
    ...
    if reciprocal:
        ...
        try:
            await bot.send_message(
                target_user_id,
                "❤️ Твоя анкета кому-то понравилась!\n\n"
                "Если ты тоже поставишь этому пользователю ❤️, "
                "у вас будет взаимная симпатия.",
                reply_markup=like_notification_keyboard(
                    rater_id
                ),
            )
        except Exception:
...
async def send_match_notification(
    ...
    username_a = format_username(
        user_a
    )

    username_b = format_username(
        user_b
    )

    text_for_a = (
        "💘 ВЗАИМНАЯ СИМПАТИЯ!\n\n"
        f"❤️ Ты понравился(ась) {username_b}.\n\n"
    ...
    text_for_b = (
        "💘 ВЗАИМНАЯ СИМПАТИЯ!\n\n"
        f"❤️ Ты понравился(ась) {username_a}.\n\n"
        "Теперь вы можете связаться друг с другом "
        "в Telegram."
    )
    ...
    try:
        ...
        logging.exception(
            "Failed to send match notification to user A"
        )
    ...
    try:
        ...
        logging.exception(
            "Failed to send match notification to user B"
        )
