...
# ============================================================
# DATING / LIKE NOTIFICATIONS
# ============================================================

def profile_notification_keyboard(user_id: int):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
...
def mutual_match_keyboard():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👤 Открыть анкету",
                    callback_data="match_profile",
                )
            ],
            [
                InlineKeyboardButton(
                    text="➡️ Рейтить дальше",
                    callback_data="menu_rate",
                )
            ],
        ]
...
def telegram_contact(user: dict):
...
async def next_profile(
    ...
    current_profile_id = current_data.get(
        "rating_profile_user_id"
    )

    seen_profile_ids = (
        current_data.get("seen_profile_ids")
        or []
    )

    # Обязательно добавляем текущую анкету
    # в список просмотренных ДО поиска следующей.
    if current_profile_id:
        if current_profile_id not in seen_profile_ids:
            seen_profile_ids.append(
                current_profile_id
            )
...
async def show_next_profile(
    ...
    seen_profile_ids = (
        data.get("seen_profile_ids")
        or []
    )

    # Сначала ищем ещё не оценённую анкету.
    profile = await db.get_random_unrated_profile(
        user_id,
        exclude_ids=seen_profile_ids,
    )
    ...
    # Если новые закончились, ищем уже оценённые,
    # но всё равно исключаем ВСЕ просмотренные
    # в текущей сессии.
    ...
    if profile:
        ...
        text = (
            ...
            "Показываю анкеты, которые ты уже оценивал."
        )
    ...
    await message.answer(
        "😔 Других доступных анкет сейчас нет.",
        reply_markup=main_menu(),
    )
...
async def like_current(
    ...
    if existing:
        await callback.answer(
            "❤️ Ты уже ставил лайк этой анкете.",
            show_alert=True,
        )
        return
    ...
    if mutual:
        ...
        await callback.message.answer(
            ...
            "Контакты уже отправлены вам обоим.",
    ...
    await callback.message.answer(
        "❤️ Лайк отправлен!\n\n"
        "Если этот человек тоже поставит "
        "тебе лайк — вы получите контакты друг друга.",
        reply_markup=after_rating_keyboard(),
    )
...
async def notify_like(
    ...
    try:
        liker_profile = await db.get_profile(
            liker_id
        )

        liker_user = await db.get_user(
            liker_id
        )
        ...
        name = (
            f"@{html.escape(liker_user['username'])}"
            if liker_user.get("username")
            else "Кто-то"
        )

        text = (
            "❤️ Тебе поставили лайк!\n\n"
            f"{name} отметил(а) твою анкету.\n\n"
            "Посмотри анкету. Если он(а) "
            "тебе тоже понравится — поставь лайк "
            "и вы познакомитесь."
        )
        ...
        await bot.send_message(
            target_user_id,
            text,
            reply_markup=profile_notification_keyboard(
                liker_id
            ),
        )
...
async def send_mutual_match(
    ...
    try:
        user_a = await db.get_user(
            user_a_id
        )

        user_b = await db.get_user(
            user_b_id
        )
        ...
        contact_a = telegram_contact(user_a)
        contact_b = telegram_contact(user_b)
        ...
        await bot.send_message(
            ...
            text_for_a,
            parse_mode="HTML",
            reply_markup=mutual_match_keyboard(),
        )
        ...
        logging.exception(
            "Failed to send mutual match notification"
        )
...
@dp.callback_query(
    F.data.startswith("view_liker:")
)
async def view_liker(
    callback: CallbackQuery,
    state: FSMContext,
):
    try:
...
@dp.callback_query(F.data == "match_profile")
async def match_profile(
    callback: CallbackQuery,
):
    # Кнопка просто открывает собственную анкету
...
async def cancel_rating_change(
    ...
    current_profile_id = data.get(
        "rating_profile_user_id"
    )

    if (
        current_profile_id
        and current_profile_id not in seen_profile_ids
    ):
        seen_profile_ids.append(
            current_profile_id
        )
...
# ============================================================
# LIKE FROM NOTIFICATION PROFILE
# ============================================================

@dp.callback_query(
    F.data.startswith("like_user:")
)
async def like_user_from_profile(
    callback: CallbackQuery,
):
    try:
        target_user_id = int(
    ...
    await callback.message.answer(
        "❤️ Лайк отправлен!\n\n"
        "Если человек ответит взаимным лайком, "
        "вы получите контакты друг друга.",
        reply_markup=main_menu(),
    )
