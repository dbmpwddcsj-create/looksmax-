...
MAX_PHOTOS = 5
MENU_PHOTO_URL = "https://i.imgur.com/Ah1m7LS.jpg"
...
def male_look_keyboard():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Sub 3", callback_data="look:sub3"),
                InlineKeyboardButton(text="Sub 5", callback_data="look:sub5"),
            ],
            [
                InlineKeyboardButton(text="LTN", callback_data="look:ltn"),
                InlineKeyboardButton(text="MTN", callback_data="look:mtn"),
                InlineKeyboardButton(text="HTN", callback_data="look:htn"),
            ],
            [
                InlineKeyboardButton(text="Chad", callback_data="look:chad"),
                InlineKeyboardButton(text="True Adam", callback_data="look:true_adam"),
            ],
            [
                InlineKeyboardButton(text="⏭ Пропустить", callback_data="look:skip")
            ],
        ]
    )
...
def female_look_keyboard():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="LTB", callback_data="look:ltb"),
                InlineKeyboardButton(text="MTB", callback_data="look:mtb"),
                InlineKeyboardButton(text="HTB", callback_data="look:htb"),
            ],
            [
                InlineKeyboardButton(text="Stacy", callback_data="look:stacy"),
                InlineKeyboardButton(text="True Eve", callback_data="look:true_eve"),
            ],
            [
                InlineKeyboardButton(text="⏭ Пропустить", callback_data="look:skip")
            ],
...
def liked_profile_keyboard():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👤 Посмотреть анкету",
                    callback_data="view_liker",
                )
            ],
            [
                InlineKeyboardButton(
...
def mutual_like_keyboard(user_id: int):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="👤 Посмотреть анкету",
                    callback_data=f"view_liker:{user_id}",
                )
            ],
            [
                InlineKeyboardButton(
...
async def notify_like(
    ...
    try:
        liker_username = await get_username(liker_id)

        if mutual:
            liked_username = await get_username(liked_user_id)

            await bot.send_message(
                liked_user_id,
                "❤️ Взаимная симпатия!\n\n"
                f"Тебе понравился пользователь {liker_username}, "
                "и ты тоже понравился ему.\n\n"
                f"Его Telegram: {liker_username}",
                reply_markup=mutual_like_keyboard(liker_id),
            )

            await bot.send_message(
                liker_id,
                "❤️ Взаимная симпатия!\n\n"
                f"Тебе понравился пользователь {liked_username}, "
                "и ты тоже понравился ему.\n\n"
                f"Его Telegram: {liked_username}",
                reply_markup=mutual_like_keyboard(liked_user_id),
            )
        else:
            await bot.send_message(
                liked_user_id,
                "❤️ Ты кому-то понравился!\n\n"
                f"Пользователь {liker_username} поставил тебе лайк.",
                reply_markup=liked_profile_keyboard(),
            )
    except Exception:
        logging.exception("Failed to send like notification")
...
async def show_menu(message: Message):
    ...
    if profile and profile.get("status") == "active":
        text = (
            ...
            "ставить лайки и получать советы.\n\n"
            "💫 Всё бесплатно!\n\n"
            "Что хочешь сделать?"
        )
    else:
    ...
    try:
        ...
        logging.exception("Failed to send menu photo")
...
async def get_photo_ids(profile: dict):
    ...
    return [photo_id] if photo_id else []
...
async def save_profile_from_state(user_id: int, state: FSMContext):
    ...
    if existing:
        await db.update_profile(user_id, profile_data)
    else:
...
async def profile_photo(message: Message, state: FSMContext):
    ...
    await message.answer(
        (
            ...
            if len(photo_ids) == 1
            else "✅ Фотография добавлена.\n\n"
        )
        + f"Добавлено: {len(photo_ids)}/{MAX_PHOTOS}\n\n"
        "Можешь добавить ещё фотографии или нажать «Готово».",
        reply_markup=photo_collection_keyboard(),
    )
...
@dp.callback_query(ProfileCreation.photo, F.data == "add_photo")
async def add_photo(callback: CallbackQuery):
    await callback.message.answer("📸 Отправь следующую фотографию.")
    await callback.answer()


@dp.callback_query(ProfileCreation.photo, F.data == "photos_done")
...
async def skip_profile_facts(callback: CallbackQuery, state: FSMContext):
    await state.update_data(facts=None)
    await ask_profile_height(callback.message, state)
    await callback.answer()
...
async def profile_facts(message: Message, state: FSMContext):
    ...
    await ask_profile_height(message, state)


async def ask_profile_height(message: Message, state: FSMContext):
    await message.answer(
...
async def skip_profile_height(callback: CallbackQuery, state: FSMContext):
    await state.update_data(height=None)
    await ask_profile_weight(callback.message, state)
    await callback.answer()
...
async def profile_height(message: Message, state: FSMContext):
    ...
    if text.lower() in {"нет", "no", "-", "пропустить"}:
        ...
        if height < 100 or height > 250:
            await message.answer("❌ Укажи рост от 100 до 250 см.")
            return

    await state.update_data(height=height)
    await ask_profile_weight(message, state)


async def ask_profile_weight(message: Message, state: FSMContext):
    await message.answer(
...
async def skip_profile_weight(callback: CallbackQuery, state: FSMContext):
...
async def profile_weight(message: Message, state: FSMContext):
    ...
    if text.lower() in {"нет", "no", "-", "пропустить"}:
        ...
        if weight < 30 or weight > 300:
            await message.answer("❌ Укажи вес от 30 до 300 кг.")
            return
...
async def finish_profile_creation(
    ...
    saved = await save_profile_from_state(user_id, state)
...
async def my_profile(callback: CallbackQuery):
    ...
    await send_profile(callback.message, profile, admin=False)
    await callback.answer()


async def build_profile_text(profile: dict):
    user = await db.get_user(profile["user_id"])

    if not user:
        return None

    average = await db.get_average_rating(profile["user_id"])
    count = await db.get_rating_count(profile["user_id"])
...
async def send_profile(
    ...
    text = await build_profile_text(profile)

    if text is None:
        return

    photo_ids = await get_photo_ids(profile)

    keyboard = (
        admin_profile_keyboard(profile["user_id"])
        if admin
        else profile_keyboard()
    )
...
async def delete_profile_question(callback: CallbackQuery):
    await callback.message.answer(
...
async def edit_age_receive(message: Message, state: FSMContext):
    try:
        age = int((message.text or "").strip())
    except ValueError:
        await message.answer(
...
async def edit_gender_receive(callback: CallbackQuery, state: FSMContext):
    gender = "Мужской" if callback.data == "gender_male" else "Женский"
...
@dp.callback_query(ProfileEdit.photos, F.data == "add_photo")
async def edit_add_photo(callback: CallbackQuery):
    await callback.message.answer("📸 Отправь следующую фотографию.")
    await callback.answer()


@dp.callback_query(ProfileEdit.photos, F.data == "photos_done")
...
async def show_next_profile(
    ...
    profile = await db.get_random_unrated_profile(user_id)
    ...
    profile = await db.get_random_rated_profile(user_id)
...
async def like_current(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    liked_user_id = data.get("rating_profile_user_id")
    ...
    if liked_user_id == callback.from_user.id:
        await callback.answer(
    ...
    profile = await db.get_profile(liked_user_id)
    ...
    already_liked = await db.has_like(
        callback.from_user.id,
        liked_user_id,
    )
    ...
    if already_liked:
        await callback.answer(
            "❤️ Ты уже поставил этому пользователю лайк.",
            show_alert=True,
        )
        return
    ...
    if mutual:
        ...
        await callback.message.answer(
            "❤️ Лайк поставлен!\n\n"
            "Если пользователь тоже поставит тебе лайк, "
            "вы получите уведомление о взаимной симпатии.",
            reply_markup=after_rating_keyboard(),
        )
...
# ============================================================
# LIKES MENU / VIEW LIKER
# ============================================================

@dp.callback_query(F.data == "menu_likes")
async def menu_likes(callback: CallbackQuery, state: FSMContext):
    likes = await db.get_likes_received(callback.from_user.id)

    if not likes:
        await callback.message.answer(
            "❤️ Пока никто не поставил тебе лайк.",
            reply_markup=main_menu(),
        )
        await callback.answer()
        return
    ...
    for like in likes[:20]:
        liker_id = like.get("user_id")
        if not liker_id:
            continue

        profile = await db.get_profile(liker_id)

        if not profile or profile.get("status") != "active":
            continue

        user = await db.get_user(liker_id)
...
@dp.callback_query(F.data == "view_liker")
async def view_liker_from_notification(
    callback: CallbackQuery,
    state: FSMContext,
):
    # The notification button is tied to the latest notification.
    # If Telegram callback data cannot contain that context, the
    # user can use the "Нравится" section in the main menu.
    await callback.answer(
        "Открой «❤️ Нравится» в главном меню, чтобы посмотреть лайки.",
        show_alert=True,
    )
...
async def send_profile_for_like(
    message: Message,
    profile: dict,
    viewer_id: int,
):
    ...
    await send_photos(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="❤️ Нравится в ответ",
                        callback_data=f"like_back:{profile['user_id']}",
                    )
                ],
                [
                    InlineKeyboardButton(
...
async def like_back(callback: CallbackQuery):
    ...
    await callback.answer(
        "❤️ Взаимная симпатия!" if mutual else "❤️ Лайк поставлен!",
        show_alert=True,
    )


# ============================================================
# ENTER SCORE / LOOK TYPE
# ============================================================
...
async def receive_score(message: Message, state: FSMContext):
    score = parse_number(message.text or "")
    ...
    await state.update_data(pending_score=score)
    data = await state.get_data()

    profile_user_id = data.get("rating_profile_user_id")
...
async def ask_look_type(message: Message, state: FSMContext):
    data = await state.get_data()
    profile_user_id = data.get("rating_profile_user_id")
    ...
    user = await db.get_user(profile_user_id)
    ...
    await state.set_state(Rating.look_type)
...
async def select_look_type(callback: CallbackQuery, state: FSMContext):
    value = callback.data.split(":", 1)[1]
    look_type = None if value == "skip" else value
    ...
    table_text = (
        f"\n📊 Тип по таблице: {format_look_type(look_type)}"
        if look_type
        else "\n📊 Оценку по таблице пропустили."
    )

    await callback.message.answer(
        f"✅ Оценка сохранена: {float(score):.1f}/10."
        f"{table_text}",
        reply_markup=after_rating_keyboard(),
    )
...
async def notify_rating(
    ...
    try:
        average = await db.get_average_rating(profile_user_id)
                ...
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            ...
                            callback_data=f"view_rater:{rater_id}",
                        )
                    ]
                ]
            ),
        ...
        logging.exception("Failed to send rating notification")
...
@dp.callback_query(F.data.startswith("view_rater:"))
async def view_rater(callback: CallbackQuery):
    user_id = int(callback.data.split(":")[1])
    ...
    await send_rater_profile(callback.message, profile)
    await callback.answer()


async def send_rater_profile(message: Message, profile: dict):
    user = await db.get_user(profile["user_id"])
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⭐ Оценить",
                        callback_data=f"rate_user:{profile['user_id']}",
                    )
                ],
                [
                    InlineKeyboardButton(
...
@dp.callback_query(F.data.startswith("rate_user:"))
async def rate_user_from_notification(
    ...
    await state.clear()
    await state.update_data(rating_profile_user_id=user_id)
...
async def advice_start(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    to_user_id = data.get("rating_profile_user_id")
    ...
    profile = await db.get_profile(to_user_id)
    ...
    await state.set_state(Rating.advice)
    await callback.answer()


@dp.message(Rating.advice)
async def advice_received(message: Message, state: FSMContext):
    text = (message.text or "").strip()
    ...
    try:
        ...
        logging.exception("Failed to send advice")
...
async def report_reason(message: Message, state: FSMContext):
    ...
    report_id = report[0]["id"] if report else None
...
async def admin_reports(callback: CallbackQuery):
    ...
    for report in reports[:30]:
        await callback.message.answer(
            ...
            f"Причина:\n{report['reason']}",
            reply_markup=admin_report_keyboard(report["id"]),
        )

    await callback.answer()


@dp.callback_query(F.data.startswith("admin_view_report:"))
async def admin_view_report(callback: CallbackQuery):
    ...
    profile = await db.get_profile(report["profile_user_id"])
    ...
    await send_profile(callback.message, profile, admin=True)
    await callback.answer()


@dp.callback_query(F.data.startswith("admin_delete_report:"))
async def admin_delete_report(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f"🗑 Анкета пользователя {user_id} удалена."
    )
    ...
    try:
        ...
        logging.exception("Failed to notify deleted user")
