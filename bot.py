...
MEN_TABLE_IMAGE = (
    ...
    "b210ff5eeae904c276ea9d49d6f365d6e527397d/IMG_1423.jpeg"
)

WOMEN_TABLE_IMAGE = (
    "https://raw.githubusercontent.com/"
    "dbmpwddcsj-create/photo/"
    "c0d698287fc20b14c88fb34edbb7401c8856cc1e/IMG_1424.jpeg"
)
...
async def like_current(callback: CallbackQuery, state: FSMContext):
    ...
    if mutual:
        ...
        # Уведомляем пользователя, что он кому-то понравился
        try:
            await bot.send_message(
                target_id,
                "❤️ Ты понравился кому-то!",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="👤 Посмотреть анкету", callback_data=f"rated_by:{callback.from_user.id}")]
                ])
            )
        except (TelegramForbiddenError, TelegramBadRequest):
            pass
