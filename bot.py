...
if not ADMIN_ID:
    raise RuntimeError(
        "ADMIN_ID is not set in Render Environment Variables"
    )
...
if not RENDER_EXTERNAL_URL:
    raise RuntimeError(
        "RENDER_EXTERNAL_URL is not set in Render Environment Variables"
    )

if not WEBHOOK_SECRET:
    raise RuntimeError(
        "WEBHOOK_SECRET is not set in Render Environment Variables"
    )
...
async def stats_text(user_id):
    scores = await db.get_scores(user_id)

    if not scores:
        return "⭐ Оценок пока нет"

    average = format_average(scores)
